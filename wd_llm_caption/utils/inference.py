        ...
        with torch.no_grad():
            ...
            if self.models_type == "joy":
                if self.args.llm_model_name == "Joy-Caption-Pre-Alpha":
                    ...
                    convo_string = self.llm_tokenizer.apply_chat_template(convo, tokenize=False, add_generation_prompt=True)
                    assert isinstance(convo_string, str)

                    # Tokenize the conversation
                    # prompt_str is tokenized separately so we can do the calculations below
                    convo_tokens = self.llm_tokenizer.encode(convo_string, return_tensors="pt", add_special_tokens=False,
                                                             truncation=False)
                    prompt_tokens = self.llm_tokenizer.encode(user_prompt, return_tensors="pt", add_special_tokens=False,
                                                              truncation=False)
                    ...
                    eot_id_indices = \
                        (convo_tokens == self.llm_tokenizer.convert_tokens_to_ids("<|eot_id|>")).nonzero(as_tuple=True)[
                            0].tolist()
                    ...
                    generate_ids = self.llm.generate(input_ids, inputs_embeds=input_embeds, attention_mask=attention_mask,
